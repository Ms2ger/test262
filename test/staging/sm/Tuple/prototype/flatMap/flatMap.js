...
let id = x => x;
assert.sameValue(method.call(#[1,#[2],3], id), #[1,2,3]);
assert.sameValue(method.call(Object(#[1,#[2],3]), id), #[1,2,3]);
...
assert.sameValue(tup2.flatMap(f2), #[1, 0, 9]);
...
assert.sameValue(tup2.flatMap(f4), #[-0, 1, -6]);
...
assert.sameValue(#[1,2,#[3,4],#[5]].flatMap(f1, { elements: [2, 4] }), #[-1, 0, -1, -1]);
...
/* Step 6.b.i. */
var f = x => #[x, x];
assert.sameValue(#[1,#[2,3],4].flatMap(f), #[1,1,#[2,3],#[2,3],4,4]);
