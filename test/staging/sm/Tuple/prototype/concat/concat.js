...
assert.sameValue(method.call(#[1,2,3,4,5,6], #[1],#[2,3]), #[1,2,3,4,5,6,1,2,3]);
assert.sameValue(method.call(Object(#[1,2,3,4,5,6]), #[1],#[2,3]), method.call(#[1,2,3,4,5,6],#[1],#[2,3]));
...
let tup = #[1,2,3];
assert.sameValue(tup.concat(), tup);
assert.sameValue(tup.concat(#[]), tup);
assert.sameValue(tup.concat([]), tup);
assert.sameValue(tup.concat(#[],#[],#[]), tup);
...
assert.sameValue(tup.concat(tup), #[1,2,3,1,2,3]);
assert.sameValue(tup.concat([1,2,3]), #[1,2,3,1,2,3]);

/* spreadable followed by non-spreadable */
assert.sameValue(tup.concat(tup, "monkeys"), #[1,2,3,1,2,3, "monkeys"]);
assert.sameValue(tup.concat([1,2,3], "monkeys"), #[1,2,3,1,2,3, "monkeys"]);

/* non-spreadable followed by spreadable */
assert.sameValue(tup.concat("monkeys", tup), #[1,2,3, "monkeys", 1,2,3]);
assert.sameValue(tup.concat("monkeys", [1,2,3]), #[1,2,3, "monkeys", 1,2,3,]);
...
assert.sameValue(tup.concat([42,17,,5,,12]), #[1,2,3,42,17,5,12]);
...
assert.sameValue(tup.concat("monkeys"), #[1,2,3, "monkeys"]);
/*
Step 5d.c.ii.
